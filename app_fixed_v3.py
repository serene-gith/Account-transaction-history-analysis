...
with st.sidebar:
    ...
    WINDOW_DAYS = st.number_input("기간 허용(일)", 0, 60, 7, 1)
    AMOUNT_TOL_RATIO = st.number_input("금액 허용비율(%)", 0.0, 10.0, 2.0, 0.1) / 100.0
    AMOUNT_TOL_ABS = st.number_input("금액 절대허용(원)", 0, 10_000_000, 10_000, 1_000)
    GIFT_NET_THRESHOLD = st.number_input("증여의심(순유출 기준, 원)", 0, 10_000_000_000, 10_000_000, 1_000_000)
    st.markdown("---")
    st.subheader("GitHub에서 가져오기")
    repo_url = st.text_input("저장소 URL",
...
with tab1:
    ...
    if up is not None:
        uploaded_bytes = up.read(); uploaded_name = up.name
...
def list_repo_xlsx(repo_url: str, branch: str, folder_path: str, token: Optional[str]):
    parts = repo_url.strip("/").split("/"); owner, repo = parts[-2], parts[-1]
    ...
    r = requests.get(api_url, params=params, headers=headers, timeout=30); r.raise_for_status()
    items = r.json(); items = items if isinstance(items, list) else [items]
    excel_items = [{"name": it["name"], "path": it["path"]} for it in items
                   if it.get("type")=="file" and it.get("name","").lower().endswith((".xlsx",".xlsm",".xls"))]
...
def fetch_file_bytes(owner: str, repo: str, branch: str, file_path: str, token: Optional[str]):
    ...
    if r.status_code == 404:
        ...
        params = {"ref": branch}; api_headers = {"Accept":"application/vnd.github+json"}
        if token: api_headers["Authorization"] = f"token {token}"
        res = requests.get(api_url, params=params, headers=api_headers, timeout=60); res.raise_for_status()
        download_url = res.json().get("download_url")
        if not download_url: raise RuntimeError("download_url을 가져오지 못했습니다.")
...
with tab2:
    if st.button("🔍 목록 불러오기", use_container_width=True) and repo_url.strip():
        try:
            st.session_state["gh_ctx"] = list_repo_xlsx(repo_url, branch, folder_path, gh_token or None)
    ...
    if st.session_state.get("gh_ctx"):
        ...
        if not excel_list: st.warning("엑셀 파일이 없습니다. 브랜치/폴더 경로를 확인하세요.")
        else:
            ...
            if st.button("⬇️ 이 파일 가져오기", type="primary"):
                try:
                    ...
                    uploaded_bytes = data; uploaded_name = pick
                    st.session_state["uploaded_bytes"] = data; st.session_state["uploaded_name"] = pick
...
# ---------- Column mapping & analysis helpers (same as v2) ----------
SCAN_HEADER_ROWS = 20
SYN = {
    "date": ["거래일","거래일자","일자","date","transdate","날짜"],
    "time": ["거래시각","시간","time","시각"],
    "owner": ["계좌주","예금주","소유자","owner","accountowner"],
    "owner_acct": ["계좌번호","계좌","account","owneraccount","fromaccount","출금계좌","출금계좌번호"],
    "cp_holder": ["상대계좌주","상대방","거래상대","수취인","받는이","받는사람","입금계좌주","counterparty","거래상대방"],
    "cp_acct": ["상대계좌번호","상대계좌","counterpartyaccount","입금계좌","입금계좌번호","받는계좌","받는계좌번호"],
    "desc": ["거래내역","적요","메모","내용","비고","summary","description","내역","비고거래내용","비고추가"],
    "amount": ["금액","거래금액","이체금액","amount","amt","금액(원)","거래대금","거래대금원","입금금액","출금금액","입금원","출금원"],
    "credit": ["입금","입금액","수취금액","받은금액","credit","cr"],
    "debit": ["출금","출금액","송금액","보낸금액","debit","dr"],
    "method": ["거래방법","구분","입출금구분","거래구분"],
...
def _clean(s:str)->str:
    s=str(s).replace("\n","").replace("\r",""); s=re.sub(r"\s+","",s); s=s.replace("(","").replace(")",""); return s.lower()
def _to_number(x):
...
def _best_match(col:str,candidates:List[str])->Optional[str]:
    ...
    for c in candidates:
        if c in col or col in c: return c
    m=difflib.get_close_matches(col, candidates, n=1, cutoff=0.72)
    return m[0] if m else None

def _find_header(df: pd.DataFrame)->pd.DataFrame:
    if not df.empty and all(isinstance(c,str) for c in df.columns): return df
    best_row, best_score=None,-1
    for r in range(min(SCAN_HEADER_ROWS,len(df))):
        cleaned=[_clean(v) for v in df.iloc[r].astype(str).tolist()]
        tokens=sum(1 for v in cleaned if _best_match(v, sum(SYN.values(), [])) is not None)
        if tokens>best_score: best_score, best_row=tokens, r
    if best_row is None: return df
    new_cols=[_clean(x) for x in df.iloc[best_row].tolist()]
    df2=df.iloc[best_row+1:].copy(); df2.columns=new_cols; df2.reset_index(drop=True, inplace=True); return df2
...
def _map_columns(cols: List[str])->Dict[str,Optional[str]]:
    def pick(keys:List[str])->Optional[str]:
        for key in keys:
            ...
            for col in cols:
                if _best_match(col,cands): return col
        return None
    return {"owner":pick(["owner"]), "owner_acct":pick(["owner_acct"]), "cp_holder":pick(["cp_holder"]),
            "cp_acct":pick(["cp_acct"]), "amount":pick(["amount"]), "desc":pick(["desc"]), "date":pick(["date"]),
            "time":pick(["time"]), "method":pick(["method"]), "from_holder":pick(["from_holder"]),
            "from_acct":pick(["from_acct"]), "to_holder":pick(["to_holder"]), "to_acct":pick(["to_acct"]),
            "credit":pick(["credit"]), "debit":pick(["debit"])}

def _infer_direction(val:str)->Optional[str]:
    if not isinstance(val,str): return None
    if "입" in val: return "credit"
    if "출" in val: return "debit"
    return None

CP_PATTERNS=[r"(?:상대|거래상대|상대계좌주|입금계좌주|받는사람|수취인|받는분)[:：\s]\s*([^\s,/;|]+)",
             r"(?:보낸사람|송금인|출금계좌주)[:：\s]\s*([^\s,/;|]+)",
             r"(?:상대계좌번호|입금계좌|받는계좌)[:：\s]\s*([\d\-]+)"]

def _extract_counterparty(desc:str)->Dict[str,Optional[str]]:
    if not isinstance(desc,str): return {"cp_holder":None,"cp_acct":None}
    holder=None; acct=None
    for pat in CP_PATTERNS:
        m=re.search(pat, desc)
        if m:
            if "계좌" in pat and m.group(1): acct=m.group(1)
            else: holder=holder or m.group(1)
...
def load_and_standardize_from_bytes(b:bytes)->Tuple[pd.DataFrame,Dict[str,str],List[str]]:
    ...
    cols=[_clean(c) for c in df.columns]; mapping=_map_columns(cols)
    if mapping["date"] and mapping["time"] and mapping["time"] in df.columns:
        date_col=pd.to_datetime(pd.to_datetime(df[mapping["date"]], errors="coerce").dt.date.astype(str)+" "+df[mapping["time"]].astype(str), errors="coerce")
    else:
        date_col=pd.to_datetime(df[mapping["date"]], errors="coerce") if mapping["date"] else pd.NaT
    has_single_amount = bool(mapping["amount"] and mapping["amount"] in cols)
    has_split_amounts = bool((mapping["credit"] in cols if mapping["credit"] else False) or (mapping["debit"] in cols if mapping["debit"] else False))
    ...
    if all(mapping.get(k) for k in ["from_holder","from_acct","to_holder","to_acct"]) and (has_single_amount or has_split_amounts):
        ...
        if has_single_amount:
            df_std["amount"]=df[mapping["amount"]].apply(_to_number); amount_info=f"amount ← {mapping['amount']}"
        else:
            ...
            df_std["amount"]=cr.where(cr.notna()&(cr!=0),0)-dr.where(dr.notna()&(dr!=0),0); amount_info=f"amount ← +{mapping.get('credit')} -{mapping.get('debit')}"
        schema="B(from/to)"
        ...
        if cp_holder is None or (isinstance(cp_holder,pd.Series) and cp_holder.isna().all()):
            ext=desc_col.apply(_extract_counterparty); cp_holder=ext.apply(lambda d:d["cp_holder"]); cp_acct=ext.apply(lambda d:d["cp_acct"])
        if has_single_amount:
            ...
            if mdir is not None:
                ...
                amt_signed[sign==1]=abs(amt_signed[sign==1])
                amt=amt_signed
            amount_info=f"amount ← {mapping['amount']} (method used: {mapping.get('method')})"
        else:
            cr=df[mapping["credit"]].apply(_to_number) if mapping["credit"] else 0.0
            dr=df[mapping["debit"]].apply(_to_number) if mapping["debit"] else 0.0
        ...
        df_std=pd.DataFrame({"date":date_col,"from_holder":from_holder,"from_acct":from_acct,"to_holder":to_holder,"to_acct":to_acct,"amount":amt,"desc":desc_col})
        schema="A(owner/desc-augmented)"
    elif all(k in df.columns for k in ["date","from_holder","from_acct","to_holder","to_acct","amount"]):
        df_std=pd.DataFrame({"date":pd.to_datetime(df["date"], errors="coerce"),
        ...
        amount_info="already standardized"; schema="C(standardized)"
    else:
        raise ValueError(f"컬럼 자동 인식 실패. 감지된 컬럼: {cols}")
    df_std=df_std.dropna(subset=["from_holder","to_holder","amount"]).copy()
    if df_std["date"].notna().any(): df_std.sort_values(["date"], inplace=True, kind="mergesort")
    df_std.reset_index(drop=True, inplace=True)
    mapping_summary={"schema_detected":schema,"amount_info":amount_info if 'amount_info'in locals() else "", **mapping}
...
def _amount_close(a:float,b:float,tol_abs:float,tol_ratio:float)->bool:
    ...
    if abs(a-b)<=tol_abs: return True
    base=max(abs(a),abs(b),1.0); return abs(a-b)/base<=tol_ratio

def find_matches(df: pd.DataFrame)->List[Match]:
    matches=[]; used=set(); df2=df.copy()
    ...
    for key,g in df2.groupby("pair_key", sort=False):
        ...
        a,b=list(key); pos=g[(g["from_holder"]==a)&(g["to_holder"]==b)]; neg=g[(g["from_holder"]==b)&(g["to_holder"]==a)]
        for i in pos.index:
            if i in used: continue
            for j in neg.index:
                if j in used: continue
                if not _amount_close(df.loc[i,"amount"], df.loc[j,"amount"], AMOUNT_TOL_ABS, AMOUNT_TOL_RATIO): continue
                d1,d2=df.loc[i,"date"], df.loc[j,"date"]
                if pd.notna(d1) and pd.notna(d2):
                    if abs((d2-d1).days)>WINDOW_DAYS: continue
                    reason=f"금액·기간 유사(≤{WINDOW_DAYS}일)"
                else:
                    reason="금액 유사(날짜 없음)"
                used.update([i,j]); matches.append(Match(i=i,j=j,reason=reason)); break
    return matches

def build_owner_sheets(df: pd.DataFrame, matches: List[Match]):
    match_id=pd.Series(index=df.index, dtype="Int64"); reason=pd.Series(index=df.index, dtype="string")
    for k,m in enumerate(matches, start=1):
        match_id.loc[m.i]=k; match_id.loc[m.j]=k; reason.loc[m.i]=m.reason; reason.loc[m.j]=m.reason
    df2=df.copy(); df2["match_id"]=match_id; df2["match_reason"]=reason
    df2["counterparty_holder"]=df2["to_holder"]; df2["counterparty_account"]=df2["to_acct"]
    owners=sorted(set(df2["from_holder"]).union(set(df2["to_holder"])))
    sheets={}; base_cols=["date","from_holder","from_acct","to_holder","to_acct","counterparty_holder","counterparty_account","amount","desc","match_id","match_reason"]
    for owner in owners:
        sub=df2[(df2["from_holder"]==owner)|(df2["to_holder"]==owner)].copy()
        sub=sub[sub["from_holder"]!=sub["to_holder"]]
        sub=sub.sort_values(["date","amount"], kind="mergesort") if sub["date"].notna().any() else sub.sort_values(["amount"], kind="mergesort")
        present=[c for c in base_cols if c in sub.columns]; sheets[owner]=sub[present].reset_index(drop=True)
    return sheets, df2

def make_summary(df: pd.DataFrame)->pd.DataFrame:
    cols=["holder_A","holder_B","A->B_total","B->A_total","net_A_to_B","possible_donor","possible_receiver","gift_risk_hint"]
    if df.empty:
    ...
    for key,g in pairs.groupby("pair_key"):
        ...
        recs.append({"holder_A":a,"holder_B":b,"A->B_total":a2b,"B->A_total":b2a,"net_A_to_B":net,
                     "possible_donor": donor if suspicious else "", "possible_receiver": receiver if suspicious else "",
                     "gift_risk_hint": "순유출 과다" if suspicious else ""})
    out=pd.DataFrame(recs)
    return out.sort_values("net_A_to_B", ascending=False, kind="mergesort") if not out.empty else out
...
def render_and_download(df: pd.DataFrame, sheets: Dict[str, pd.DataFrame], summary: pd.DataFrame, mapping_summary: Dict[str,str], detected_cols: List[str])->bytes:
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        ...
        for owner,df_owner in sheets.items():
            sheet_name=owner[:31] or "Unknown"; df_owner.to_excel(writer, index=False, sheet_name=sheet_name)
        df.to_excel(writer, index=False, sheet_name="_Master")
        map_df=pd.DataFrame([mapping_summary]); det_df=pd.DataFrame({"detected_columns": detected_cols})
    ...
    for ws in wb.worksheets:
        if ws.title in ("Mapping","Summary"):
            limit=60 if ws.title=="Mapping" else 40
            for col in range(1, ws.max_column+1):
                max_len=max(len(str(ws.cell(row=r,column=col).value or "")) for r in range(1, ws.max_row+1))
                ws.column_dimensions[get_column_letter(col)].width=min(max_len+2, limit)
            continue
...
if run:
    ...
    if not uploaded_bytes:
        st.error("분석할 파일이 없습니다. 업로드하거나 GitHub에서 선택하세요."); st.stop()
    try:
        df, mapping_summary, detected_cols = load_and_standardize_from_bytes(uploaded_bytes)
        matches = find_matches(df); sheets, master = build_owner_sheets(df, matches)
        ...
        st.download_button("💾 결과 XLSX 다운로드", data=result_bytes, file_name=fname,
                           mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        st.dataframe(summary, use_container_width=True, height=300)
    except Exception as e:
        st.exception(e); st.error("분석 중 오류가 발생했습니다. Mapping 시트를 참고해 컬럼명을 확인해 주세요.")
